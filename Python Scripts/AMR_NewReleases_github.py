...
import csv
import sys # for Zvuk
# from yandex_music import Client # for YM - Disabled due to regional restrictions
...
search_result = ''
# client = Client(YM_TOKEN).init() # Disabled due to regional restrictions
...
# YM ------------------------------------- (Disabled due to regional restrictions)
def send_search_request_ym(query, year):
    # Disabled: Yandex Music API not available in GitHub Actions region
    return None
...
def search_command_zv(arg_query):
    ...
    try:
        ...
        # Don't exit, just return None to continue processing
        print(f"Zvuk search error: {e}")
        return None
...
def search_album_zv(query):
    ...
    if releases is None:  # Add null check
        return None
    for rel in releases:
